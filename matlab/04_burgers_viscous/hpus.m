...
for j=2:N-1
    ...
    if ubarra1 > 0
        if (phi(j+1)-phi(j-1))==0
            ...
            if cond>=0 && cond<=1
                aux1=phi(j-1)+(phi(j+1)-phi(j-1))*(cond)*(-4*cond^4+10*cond^3-8*cond^2+cond+2);
                aux1 = ubarra1*aux1;
            else
                aux1 = ubarra1*phi(j);
            end
        ...
        if j==N-1
            ...
            if (phi(j)-phi(j+2))==0
                ...
                if cond>=0 && cond<=1
                    aux1=phi(j+2)+(phi(j)-phi(j+2))*(cond)*(-4*cond^4+10*cond^3-8*cond^2+cond+2);
                    aux1 = ubarra1*aux1;
                else
                    aux1 = ubarra1*phi(j+1);
                end
    ...
    if ubarra2 > 0
        if j==2
            ...
            if (phi(j)-phi(j-2))==0
                ...
                if cond>=0 && cond<=1
                    aux2=phi(j-2)+(phi(j)-phi(j-2))*(cond)*(-4*cond^4+10*cond^3-8*cond^2+cond+2);
                    aux2 = ubarra2*aux2;
                else
                    aux2 = ubarra2*phi(j-1);
                end
        ...
        if (phi(j-1)-phi(j+1))==0
            ...
            if cond>=0 && cond<=1
                aux2=phi(j+1)+(phi(j-1)-phi(j+1))*(cond)*(-4*cond^4+10*cond^3-8*cond^2+cond+2);
                aux2 = ubarra2*aux2;
            else
                aux2 = ubarra2*phi(j);
            end
